/**
 * from https://www.shadertoy.com/view/MsXGDj
 *
 * @bug
 * ptx is generated but :
 *  Driver API error = 0209 "CUDA_ERROR_NO_BINARY_FOR_GPU"
 *  from file <src/cudacompil.cpp>, line 147
 **/
...
#define HASHSCALE .1031f

_gpu_ float Hash(float p)
{
...
//--------------------------------------------------------------------------------
_gpu_ float Cubic(float x, float v0,float v1, float v2,float v3)
{
   float p = (v3 - v2) - (v0 - v1);
...
//--------------------------------------------------------------------------------
_gpu_ float Catmull_Rom(float x, float v0,float v1, float v2,float v3)
{
...
//--------------------------------------------------------------------------------
_gpu_ float ThirdOrderSpline(float x, float L1,float L0, float H0,float H1)
{
   return        L0 +.5 *
         x * ( H0-L1 +
         x * ( H0 + L0 * -2.0 +  L1 +
...
//--------------------------------------------------------------------------------
_gpu_ float Cosine(float x, float v0, float v1)
{
...
//--------------------------------------------------------------------------------
_gpu_ float Linear(float x, float v0, float v1)
{
   return (v1-v0)*x + v0;
}

//--------------------------------------------------------------------------------
_gpu_ float Smoothstep(float x, float v0, float v1)
{
   x = x*x*(3.0-2.0*x);
   return (v1-v0)*x + v0;
}
...
//void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
   ...
   vec4 fragColor = vec4(vec3(1.0,.2, .05) * f, 1.0);
   Output4(pixels,fragColor);
}
